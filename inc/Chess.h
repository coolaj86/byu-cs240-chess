...
///@brief Set the log levels (using the Glib log levels) that will be ignored by the Chess logging function.
///
///This mask will hide messages from being displayed in the ChessGui message box, it will not prevent them
///from being sent to the command line when not using the GUI.  Set this flag at compile time using
///the compiler directive -D and OR-ed values of the Glib log levels. For example:
///@code g++ -g -o Chess.o (Other options) -DLOG_LEVEL_HIDE_MASK=(G_LOG_LEVEL_DEBUG|G_LOG_LEVEL_INFO) Chess.cpp @endcode
...
///
///The Student will implement each of the signal handlers found in this class.  It is permitted to add
///member data and functions. The constructor and the destructor should be finished to suit the needs
///of the student.
///@note Access the @link chessguiconv convenience functions @endlink using the pointer Chess::gui
///@ingroup backend 
class Chess
{
	private:
